...
三个协作的部分（详见 [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md)）：

1. **`interceptor.js`**（MAIN world）—— 给 `window.fetch` 打补丁，被动观察 X 自己的
   GraphQL 流量。若你访问过收藏夹页面，它会捕获最精确的请求模板与响应（最高保真），且
   永不破坏原请求。
2. **`background.js`**（Service Worker）—— 按需从 `abs.twimg.com` 拉取 X 的网页端 JS
   bundle，提取当前 Bookmarks 的 `queryId` 与功能开关，从而**无需**你打开收藏夹页面即可
   获取书签；X 发新版本时自动重新抓取。
3. **`content.js`**（ISOLATED world）—— 把书签缓存进 `chrome.storage.local`，用你已登录
   的会话分页拉取，并负责注入与驱动卡片。
...
Three cooperating pieces (see [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md)):

1. **`interceptor.js`** (MAIN world) — patches `window.fetch` to passively observe
   X's own GraphQL traffic. If you ever visit your bookmarks page, it captures the
   exact request template + response (highest fidelity). Always non-destructive.
2. **`background.js`** (service worker) — on demand, fetches X's web-client JS bundle
   from `abs.twimg.com` and extracts the current Bookmarks `queryId` + feature flags,
   so the extension can pull your bookmarks **without** you visiting the bookmarks
   page. Re-scrapes automatically when X ships a new build.
3. **`content.js`** (ISOLATED world) — caches bookmarks to `chrome.storage.local`,
   actively fetches them (paginated, using your live logged-in session), and
   injects/operates the card.
